...
def _parse_language_filter(raw: Optional[str]) -> Optional[List[str]]:
    """Parse a comma/space separated language-code allowlist."""
    if not raw:
        return None
    codes = [part.strip().upper() for part in re.split(r"[,\s;]+", raw) if part.strip()]
...
) -> Tuple[List[str], List[str], Dict[str, List[str]], List[Set[str]]]:
    if not include_languages:
        return language_codes, language_names, translations, inherited_sets

    include_set = {code.upper() for code in include_languages}
    include_set.add("EN")
    available = {code.upper() for code in language_codes}
    missing = sorted(include_set - available)
    if missing:
        raise ValueError(f"Unknown language code(s) in custom_i18n_languages: {', '.join(missing)}")

    indices = [i for i, code in enumerate(language_codes) if code.upper() in include_set]
    filtered_codes = [language_codes[i] for i in indices]
    filtered_names = [language_names[i] for i in indices]
    filtered_translations = {key: [values[i] for i in indices] for key, values in translations.items()}
    filtered_inherited = [inherited_sets[i] for i in indices]
    return filtered_codes, filtered_names, filtered_translations, filtered_inherited
...
def main(
    ...
    try:
        ...
        languages, language_names, translations, inherited_sets = _filter_languages(
            languages, language_names, translations, inherited_sets, include_languages
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--languages",
        default=None,
        help="Comma-separated language codes to include. EN is always included.",
    )
    ...
    main(
        ...
        args.strip_unused,
        args.verbose,
        _parse_language_filter(args.languages),
    )
else:
    try:
        Import("env")
        try:
            language_filter = env.GetProjectOption("custom_i18n_languages")
        except Exception:
            language_filter = os.environ.get("CROSSPOINT_I18N_LANGUAGES")
        main(strip_unused=True, include_languages=_parse_language_filter(language_filter))
    except NameError:
        pass
